...
export function ContentDisplay({ selectedOption }) {
  ...
  return (
    <div className={`transition-all duration-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      <div className="bg-white/90 backdrop-blur-sm rounded-xl p-6 shadow-xl max-w-md">
        <div className="flex items-center gap-4 mb-6">
          <span className="text-2xl">{typeof content[selectedOption].icon === 'string' ? content[selectedOption].icon : content[selectedOption].icon}</span>
          <h2 className="text-2xl font-bold text-[var(--secondary)]">{content[selectedOption].title}</h2>
        </div>
